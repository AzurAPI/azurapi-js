...
export const createVoicelinesAPI = ({ voiceline }: AzurAPIState) => {
  const keys = ['event', 'en', 'zh', 'jp', 'audio'];
  const fuze = (query: string) => fuse<Voiceline>(query, voiceline.state.array, keys);
  ...
  const get = (query: string, adv?: advancedOptions): Voiceline | undefined =>
    SharedAPI.search(query, voiceline.state.array, fuze, adv);
