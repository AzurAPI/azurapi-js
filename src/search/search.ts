...
import { SearchOptions, QueryOptions, SearchOptionsDefault, QueryOptionsDefault, Dataset, ObjLike, MapLike } from './search_typings';
...
export default class AOSearch {
  ...
  public search(query: string | number, key: string, options?: QueryOptions): any | never {
    ...
    if (typeof query !== 'string' || typeof query !== 'number') throw new Error('ERR_OPTS_MALFORMED Query option must be a string or a number');
    ...
    if (!this.options.keys.includes(key)) throw new Error(`ERR_OPTS_MALFORMED Key option must be an existing value included in <AOSearch>.options.keys (${this.options.keys})`);
    ...
    // Searching
    const evaledData: any[] /* bad workaround*/ = data.map(value => {
